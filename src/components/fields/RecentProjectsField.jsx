...
export default function RecentProjectsField({ field }) {
  return (
    <div className="border rounded-xl p-4 bg-white">
      <h3 className="font-semibold text-gray-800 mb-4">{field.label || "Recent projects"}</h3>
      <div className="space-y-3">
        ...
        {projects.map((p) => (
          <div key={p.name} className="flex items-center gap-3 cursor-pointer hover:bg-gray-50 rounded-lg p-2 -mx-2">
            <div className={`w-10 h-10 ${p.bg} rounded-lg flex items-center justify-center text-lg`}>
              {p.emoji}
            </div>
            ...
            <div className="flex -space-x-2">
              {p.members.map((m, i) => (
                <div key={i} className="w-6 h-6 rounded-full bg-indigo-700 text-white text-xs flex items-center justify-center border-2 border-white font-medium">
                  {m}
                </div>
              ))}
            </div>
